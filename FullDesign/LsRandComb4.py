...
# -*- coding: utf-8 -*-
"""
Created on Thu May 13 18:14:02 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_comb(nofreq =False,constant = False, random = True, optimised = False)
    np.save('RandEstimatesComb_'+str(indx[i]),means)
    np.save('RandEntropiesComb_'+str(indx[i]),entrs)
    np.save('RandWComb_'+str(indx[i]),W)
    np.save('RandPostsComb_'+str(indx[i]),posts)
    np.save('RandFreqsComb_'+str(indx[i]),optms)
