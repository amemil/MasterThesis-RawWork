...
# -*- coding: utf-8 -*-
"""
Created on Thu May 13 18:13:53 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_comb(nofreq =False,constant = True, random = False, optimised = False)
    np.save('ConstEstimatesComb_'+str(indx[i]),means)
    np.save('ConstEntropiesComb_'+str(indx[i]),entrs)
    np.save('ConstWComb_'+str(indx[i]),W)
    np.save('ConstPostsComb_'+str(indx[i]),posts)
