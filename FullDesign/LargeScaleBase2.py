...
# -*- coding: utf-8 -*-
"""
Created on Fri May  7 17:37:40 2021
...
for i in range(5):
    ...
    np.save('BaseEstimates_'+str(indx[i]),means)
    np.save('BaseEntropies_'+str(indx[i]),entrs)
    np.save('BaseW_'+str(indx[i]),W)
    np.save('BasePosts_'+str(indx[i]),posts)