...
# -*- coding: utf-8 -*-
"""
Created on Wed May 12 19:29:31 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_tau(nofreq =True,constant = False, random = False, optimised = False)
    np.save('BaseEstimatesTau_'+str(indx[i]),means)
    np.save('BaseEntropiesTau_'+str(indx[i]),entrs)
    np.save('BaseWTau_'+str(indx[i]),W)
    np.save('BasePostsTau_'+str(indx[i]),posts)