...
# -*- coding: utf-8 -*-
"""
Created on Wed May 12 17:55:00 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_A(nofreq =False,constant = True, random = False, optimised = False)
    np.save('ConstEstimatesA_'+str(indx[i]),means)
    np.save('ConstEntropiesA_'+str(indx[i]),entrs)
    np.save('ConstWA_'+str(indx[i]),W)
    np.save('ConstPostsA_'+str(indx[i]),posts)