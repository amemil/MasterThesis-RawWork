...
# -*- coding: utf-8 -*-
"""
Created on Thu May 13 18:14:09 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_comb(nofreq =True,constant = False, random = False, optimised = False)
    np.save('BaseEstimatesComb_'+str(indx[i]),means)
    np.save('BaseEntropiesComb_'+str(indx[i]),entrs)
    np.save('BaseWComb_'+str(indx[i]),W)
    np.save('BasePostsComb_'+str(indx[i]),posts)