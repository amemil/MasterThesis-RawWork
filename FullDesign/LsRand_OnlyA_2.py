...
# -*- coding: utf-8 -*-
"""
Created on Wed May 12 19:17:11 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_A(nofreq =False,constant = False, random = True, optimised = False)
    np.save('RandEstimatesA_'+str(indx[i]),means)
    np.save('RandEntropiesA_'+str(indx[i]),entrs)
    np.save('RandWA_'+str(indx[i]),W)
    np.save('RandPostsA_'+str(indx[i]),posts)
    np.save('RandFreqsA_'+str(indx[i]),optms)