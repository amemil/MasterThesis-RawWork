...
# -*- coding: utf-8 -*-
"""
Created on Wed May 12 19:17:19 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_A(nofreq =True,constant = False, random = False, optimised = False)
    np.save('BaseEstimatesA_'+str(indx[i]),means)
    np.save('BaseEntropiesA_'+str(indx[i]),entrs)
    np.save('BaseWA_'+str(indx[i]),W)
    np.save('BasePostsA_'+str(indx[i]),posts)