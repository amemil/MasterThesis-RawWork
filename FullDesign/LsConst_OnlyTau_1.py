...
# -*- coding: utf-8 -*-
"""
Created on Wed May 12 19:29:18 2021
...
for i in range(5):
    ...
    means,entrs,optms,W,posts = design.onlineDesign_wh_tau(nofreq =False,constant = True, random = False, optimised = False)
    np.save('ConstEstimatesTau_'+str(indx[i]),means)
    np.save('ConstEntropiesTau_'+str(indx[i]),entrs)
    np.save('ConstWTau_'+str(indx[i]),W)
    np.save('ConstPostsTau_'+str(indx[i]),posts)