...
# -*- coding: utf-8 -*-
"""
Created on Fri May  7 17:37:37 2021
...
for i in range(5):
    ...
    np.save('RandEstimatesDalesLaw_'+str(indx[i]),means)
    np.save('RandEntropiesDalesLaw_'+str(indx[i]),entrs)
    np.save('RandWDalesLaw_'+str(indx[i]),W)
    np.save('RandPostsDalesLaw_'+str(indx[i]),posts)
    np.save('RandFreqsDalesLaw_'+str(indx[i]),optms)
