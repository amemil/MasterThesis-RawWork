...
# -*- coding: utf-8 -*-
"""
Created on Fri May  7 17:37:31 2021
...
for i in range(5):
    design = ut.ExperimentDesign(freqs_init=np.array([100,10,20,50]),maxtime=60,trialsize=5\
    ...
    np.save('Const100Estimates_'+str(indx[i]),means)
    np.save('Const100Entropies_'+str(indx[i]),entrs)
    np.save('Const100W_'+str(indx[i]),W)
    np.save('Const100Posts_'+str(indx[i]),posts)
