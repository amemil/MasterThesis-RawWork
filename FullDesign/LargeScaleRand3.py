...
for i in range(5):
    ...
    np.save('RandEstimatesNewGrid_'+str(indx[i]),means)
    np.save('RandEntropiesNewGrid_'+str(indx[i]),entrs)
    np.save('RandWNewGrid_'+str(indx[i]),W)
    np.save('RandPostsNewGrid_'+str(indx[i]),posts)
    np.save('RandFreqsNewGrid_'+str(indx[i]),optms)
