...
# -*- coding: utf-8 -*-
"""
Created on Mon May 31 17:18:11 2021
...
for i in range(10):
    ...
    for j in range(len(w0s)):
        data = ut.SimulatedData(Ap=0.005, tau=0.02, std=0.0001,b1=-3.1, b2=-3.1, w0=w0s[j],sec = 10, binsize = 1/500.0,freq = 100)
        ...
        infer= ut.ParameterInference(s1,s2,P = 50, Usim = 100, Ualt = 200,it = 1500, infstd=0.0001, N = 2\
                                     , shapes_prior = np.array([4,5]), rates_prior = np.array([50,100]),sec=10\
                                         ,binsize = 1/500.0,taufix = 0.02,Afix = 0.005,b1est = -3.1,b2est = -3.1,w0est = w0s[j])
        sample = infer.standardMH()
        samples_temp.append(sample)
    samples.append(samples_temp)
    
np.save('Samples10sec100Hz11to20',samples)