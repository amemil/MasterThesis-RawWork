...
calculateMaps <- function(Data,stepsize,x,Maps1,Maps2,Maps3,Maps4,Maps5){
  for (i in 1:5){ 
  if (i==1) {
    Maps1 <<- append(Maps1,(which.max(unlist(lapply(x, FUN = dens, Data[i,c(1:1200)]))) - 1)*stepsize)
  }
  else if (i==2){
    Maps2 <<- append(Maps2,(which.max(unlist(lapply(x, FUN = dens, Data[i,c(1:1200)]))) - 1)*stepsize)
  }
  else if(i == 3){
    Maps3 <<- append(Maps3,(which.max(unlist(lapply(x, FUN = dens, Data[i,c(1:1200)]))) - 1)*stepsize)
  }
  else if (i==4){
    Maps4 <<- append(Maps4,(which.max(unlist(lapply(x, FUN = dens, Data[i,c(1:1200)]))) - 1)*stepsize)
  }
  else {
    Maps5 <<- append(Maps5,(which.max(unlist(lapply(x, FUN = dens, Data[i,c(1:1200)]))) - 1)*stepsize)
  }
  }
...
npySave("Maps0.0001Opposite2.npy",Maps1)
npySave("Maps0.0005Opposite2.npy",Maps2)
npySave("Maps0.001Opposite2.npy",Maps3)
npySave("Maps0.003Opposite2.npy",Maps4)
npySave("Maps0.005Opposite2.npy",Maps5)
